...
@Composable
fun FinanceExpense(navController: NavHostController, transactionViewModel: TransactionViewModel) {
    UIVar.topBarTitle = stringResource(id = R.string.financeexpenses_title)
    ...
    val transactions = transactionViewModel.expenseTransactions.collectAsState()
    ...
    val list: List<Transaction> = remember(input, currentTransactions) {
        if (input.isEmpty()) {
            ...
            val filteredList = transactionViewModel.expenseContainsList(input, context)
            filteredList
        }
    }

    Column (modifier = Modifier.padding(UIVar.Padding).verticalScroll(rememberScrollState()).fillMaxWidth()) {
        ...
        Box(modifier = Modifier.align(Alignment.BottomCenter).fillMaxWidth())
        {
            Row(modifier = Modifier, Arrangement.SpaceAround) {
                ...
                Button(onClick = { navController.popBackStack() }, modifier = Modifier.weight(1f)) {
                    Text(text = stringResource(id = R.string.general_back))
                }
                Spacer(modifier = Modifier.width(UIVar.Padding))
                Button(onClick = { navController.navigate("FinanceIncome") }, modifier = Modifier.weight(1f)) {
                    Text(text = stringResource(id = R.string.financeexpenses_toincome))
                }
                Spacer(modifier = Modifier.width(UIVar.Padding))
            }
        }
